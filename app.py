...
with st.sidebar:
    ...
    st.title("신항공장 생산관리")
    st.caption("Ver 31.0 (Backup System)")
    ...
    with st.expander("🛠️ 시스템 관리 (백업/복구)"):
        ...
        st.markdown("---")
        st.markdown("##### 🔄 데이터 복구 (Upload)")
        uploaded_db = st.file_uploader("DB 파일 복구", type=['json'], key="u_db")
        if uploaded_db:
            try:
                st.session_state.daily_db = json.load(uploaded_db)
                save_db_state()
                st.success("DB 복구 완료!")
            except: st.error("파일 오류")
            
        uploaded_log = st.file_uploader("로그 파일 복구", type=['json'], key="u_log")
        if uploaded_log:
            try:
                d = json.load(uploaded_log)
        ...
        uploaded_cont = st.file_uploader("계약서 파일 복구", type=['json'], key="u_cont")
        if uploaded_cont:
            try:
                st.session_state.contracts = json.load(uploaded_cont)
                save_contracts_state()
                st.success("계약 정보 복구 완료!")
            except: st.error("파일 오류")
