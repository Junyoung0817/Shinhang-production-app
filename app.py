...
with st.sidebar:
    ...
    st.title("신항공장 생산관리")
    st.caption("Ver 29.1 (Header Fix)")
...
# 상단 헤더 (HTML 문자열 생성 방식 수정: 들여쓰기 완전 제거)
def render_header(data, selected_dt):
    ...
    # [핵심] 들여쓰기 없이 한 줄 또는 왼쪽 벽에 붙여서 작성
    html_code = f"""
...
<div class="q-row"><span class="q-label highlight-label">Total Cl</span><span class="q-val highlight-val" style="{st_cl}">{total_cl:.1f}</span></div>
