...
with st.sidebar:
    ...
    st.title("신항공장 생산관리")
    st.caption("Ver 31.2 (Layout Final Fix)")
...
# 상단 헤더 (HTML 100% 한 줄 처리 - 들여쓰기/줄바꿈 완전 제거)
def render_header(data, selected_dt):
    ...
    # [핵심] 줄바꿈 없는 한 줄 문자열 연결 (Explicit String Concatenation)
    html_code = ""
    html_code += '<div style="background-color: white; padding: 20px 30px; border-radius: 12px; box-shadow: 0 4px 6px rgba(0,0,0,0.02); margin-bottom: 25px; border-top: 4px solid #e74c3c;">'
    html_code += '<div style="display: flex; justify-content: space-between; align-items: center;">'
    html_code += f'<div><h3 style="margin:0; color:#32325d;">2026 신항공장 생산 통합 시스템 (Pro)</h3><span style="color:#8898aa; font-size:0.9rem;">Date: {DATE_KEY}</span></div>'
    html_code += '<div style="text-align:right;"><span style="background:#d4edda; color:#155724; padding:5px 12px; border-radius:20px; font-size:0.85rem; font-weight:600;">● System Active</span></div></div>'
    html_code += '<div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; width: 100%; margin-top: 20px; padding-top: 20px; border-top: 1px solid #e9ecef;">'
    html_code += f'<div style="border-right: 1px solid #eee; padding-right: 20px;"><div style="font-size: 0.9rem; color: #11cdef; font-weight: 600; text-transform: uppercase; margin-bottom: 8px;">● 월간 PTU 생산량</div><div style="font-size: 1.8rem; font-weight: 800; color: #32325d; line-height: 1.2;">{monthly_prod:,.1f} <span style="font-size: 1.0rem; color: #8898aa; font-weight: 500;">Ton</span></div><div style="font-size:0.8rem; color:#aaa; margin-top:5px;">(TK-710 + 720 합계)</div></div>'
    ...
    html_code += f'<div><div style="font-size: 0.9rem; color: #5e72e4; font-weight: 600; text-transform: uppercase; margin-bottom: 8px;">UTK-1106 (SHORE)</div><div style="font-size: 1.8rem; font-weight: 800; color: #32325d; line-height: 1.2;">{utk_1106:,.1f} <span style="font-size: 1.0rem; color: #8898aa; font-weight: 500;">Ton</span></div></div>'
    html_code += '</div></div>'
...
if menu == "1. 통합 대시보드 (Dashboard)":
    ...
    for i, t_name in enumerate(SPECS):
        ...
        with cols[i % 3]:
            ...
            # [수정] 카드 HTML도 한 줄 연결
            card_html = ""
            card_html += '<div class="tank-card">'
            card_html += f'<div style="display:flex; justify-content:space-between; align-items:center;"><div style="font-weight:bold; font-size:1.1rem; color:#32325d;">{spec["icon"]} {t_name}</div><span style="background:{spec["color"]}20; color:{spec["color"]}; padding:2px 8px; border-radius:4px; font-size:0.75rem; font-weight:700;">{spec["type"]}</span></div>'
            card_html += f'<div style="margin-top:15px; margin-bottom:10px;"><div class="metric-value" style="font-size:1.5rem;">{d["qty"]:.1f} <span class="metric-unit">Ton</span></div></div>'
            card_html += f'<div style="margin-bottom:15px;"><div style="display:flex; justify-content:space-between; font-size:0.8rem; margin-bottom:3px; color:#8898aa;"><span>Level</span><span>{pct:.1f}%</span></div><div style="width:100%; background:#f6f9fc; height:6px; border-radius:10px;"><div style="width:{pct}%; background:{spec["color"]}; height:6px; border-radius:10px;"></div></div></div>'
            card_html += '<div class="quality-grid">'
            card_html += f'<div class="q-row"><span class="q-label">AV</span><span class="q-val" style="{st_av}">{d["av"]:.2f}</span></div>'
            card_html += f'<div class="q-row"><span class="q-label">Water</span><span class="q-val" style="{st_water}">{d["water"]:.1f}</span></div>'
            card_html += f'<div class="q-row"><span class="q-label">Total Cl</span><span class="q-val" style="{st_cl}">{total_cl:.1f}</span></div>'
            card_html += f'<div class="q-row"><span class="q-label">Total Metal</span><span class="q-val" style="{st_metal}">{d["metal"]:.1f}</span></div>'
            card_html += f'<div class="q-row"><span class="q-label" style="font-size:0.8em; padding-left:10px;">└ Org Cl</span><span class="q-val" style="font-size:0.8em;">{org_cl:.1f}</span></div>'
            card_html += f'<div class="q-row"><span class="q-label" style="font-size:0.8em; padding-left:10px;">└ InOrg Cl</span><span class="q-val" style="font-size:0.8em;">{inorg_cl:.1f}</span></div>'
            card_html += f'<div class="q-row"><span class="q-label">P</span><span class="q-val" style="{st_p}">{d["p"]:.1f}</span></div>'
            card_html += '</div></div><div style="margin-bottom:20px"></div>'
